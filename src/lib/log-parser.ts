...
export const parseLogsToDashboardData = (
    ...
    lines.forEach((line) => {
        ...
        // Video recording (🎥 Video kaydedildi: ...)
            ...
            if (filename) {
                const videoUrl = `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080"}/videos/${filename}`;
                if (currentTest) {
        ...
        // Screenshot recording (📸 Screenshot kaydedildi: ...)
            ...
            if (filename && filename.endsWith(".png")) {
                const screenshotUrl = `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080"}/screenshots/${filename}`;
                if (currentTest) {
