...
class SocketService {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        if (url.pathname && url.pathname !== "/") {
          ...
          socketPath = cleanPath.endsWith("/socket.io")
            ? cleanPath
            : `${cleanPath}/socket.io`;
        }
      } catch (e) {
        console.warn(
          "[Socket] Failed to parse socket URL, using as-is:",
          socketUrl,
        );
      }
      ...
      this.socket.on("reconnect_attempt", (attempt: number) => {
        ...
        console.log(
          `Reconnect attempt ${attempt}/${this.maxReconnectAttempts}`,
        );
      });
  ...
  joinInjectionRoom(jobId: string): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.socket.emit(
        "subscribe",
        { room },
        (response: { status: string; room: string; message?: string }) => {
          ...
          if (response.status === "subscribed") {
            ...
            reject(
              new Error(response.message || "Failed to join injection room"),
            );
          }
        },
      );
  ...
  onJobStarted(
    callback: (data: { id: string; status: string; startedAt: string }) => void,
  ) {
  ...
  onJobProgress(callback: (data: JobProgressPayload) => void) {
    this.socket?.on("job:progress", (data) => {
      console.log(
        "[Socket.ts] RAW job:progress received:",
        JSON.stringify(data),
      );
      callback(data);
    });
