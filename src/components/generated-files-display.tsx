...
export function GeneratedFilesDisplay({ data, dictionary, onSuccessAll }: GeneratedFilesDisplayProps) {
    ...
    const handleUpdateCode = (groupName: string, index: number, newCode: string) => {
        setLocalData(prev => {
            const newData = { ...prev };
            ...
            <Dialog open={showPreviewModal} onOpenChange={(open) => {
                ...
                <DialogContent className="sm:max-w-[600px] max-h-[90vh] flex flex-col">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {isInjecting || progress
                                ? fullDict.injection.progress || "Please wait while files are being injected."
                                : fullDict.injection.reviewFiles || "Review the files that will be added to your project."}
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    {isInjecting || progress ? (
                                ...
                                {progress?.progress === 100 && (
                                    ...
                                    >
                                        ...
                                        <p className="text-sm text-muted-foreground">
                                            {fullDict.injection.successMessage.replace("{count}", String(allFiles.length))}
                                        </p>
                                    </motion.div>
                                )}
                            </AnimatePresence>
                        ...
                        <>
                                    ...
                                    {fileGroups.map(([groupName, files]) => (
                                        <div key={groupName} className="space-y-2">
                                            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-500 flex items-center gap-2">
                                                ...
                                                {groupName.replace(/([A-Z])/g, " $1").trim()} ({files.length})
                                            </h4>
                                            <ul className="space-y-1 pl-3">
                                                {files.map((file, i) => (
                                                    <li key={i} className="text-sm flex items-center gap-2 text-slate-600 dark:text-slate-300 py-1 border-b border-slate-100 dark:border-slate-800 last:border-0">
                            ...
                            <DialogFooter className="gap-2 sm:gap-0">
                                <Button variant="ghost" onClick={() => setShowPreviewModal(false)} disabled={isInjecting}>
                                    {dictionary.common.cancel || fullDict.common.cancel || "Cancel"}
                                </Button>
                                <Button
                                ...
                                >
                                    {isInjecting ? (
                                        ...
                                        fullDict.injection.confirmAndInject || "Confirm & Inject All"
                                    )}
                                </Button>
                            </DialogFooter>
                        </>
                        ...
                        <div className="flex items-center justify-between">
                            <CardTitle className="text-lg flex items-center gap-2 capitalize">
                                ...
                                <span className="text-sm font-normal text-muted-foreground">
                                    ({files.length} {files.length === 1 ? fullDict.injection.file : fullDict.injection.files})
                                </span>
                            </CardTitle>
                                                    ...
                                                    {isEditing ? (
                                                        <div className="p-4 bg-slate-900">
                                                            <Textarea
                                                                ...
                                                                value={file.code}
                                                                onChange={(e) => handleUpdateCode(groupName, index, e.target.value)}
                                                            />
