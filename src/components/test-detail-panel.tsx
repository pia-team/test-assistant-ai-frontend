...
export function TestDetailPanel({ test }: TestDetailPanelProps) {
  ...
  const filteredSteps = useMemo(() => {
    return steps.filter((step) => {
      const matchesText = step.content
        .toLowerCase()
  ...
  const handleNextVideo = () => {
    setCurrentVideoIndex((prev) =>
      prev < activeVideos.length - 1 ? prev + 1 : 0,
    );
  };
        ...
        <CardHeader className="py-2 border-b flex flex-row items-center justify-between space-y-0">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-sm">
              {dictionary.testDetail.executionVideo}
            </CardTitle>
          </div>
          ...
          {activeVideos.length > 0 ? (
            <>
              <div className="w-full relative">
                ...
                >
                  <source
                    src={activeVideos[currentVideoIndex]}
                    type="video/webm"
                  />
                ...
                <Select value={statusFilter} onValueChange={setStatusFilter}>
                  ...
                  <SelectContent>
                    <SelectItem value="ALL">
                      {dictionary.testDetail.allStatus}
                    </SelectItem>
                    <SelectItem value="PASS">
                      {dictionary.testDetail.pass}
                    </SelectItem>
                    <SelectItem value="FAIL">
                      {dictionary.testDetail.fail}
                    </SelectItem>
                    <SelectItem value="INFO">
                      {dictionary.testDetail.info}
                    </SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="p-3 border-t flex flex-wrap gap-3 justify-between items-center bg-muted/30">
                <div className="flex items-center gap-2">
                  <span className="text-sm text-muted-foreground">
                    {dictionary.testDetail.rowsPerPage}
                  </span>
                  <Select
                  ...
                  <span className="text-sm text-muted-foreground ml-2">
                    ...
                    {Math.min(currentPage * itemsPerPage, filteredSteps.length)}{" "}
                ...
                {test.screenshots && test.screenshots.length > 0 ? (
                  ...
                  <div className="space-y-4">
                    <p className="text-sm text-muted-foreground mb-4">
                      {dictionary.testDetail.videoThumbnails ||
                        "Video kayıtlarından alınan görüntüler:"}
                    </p>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  ...
                  <div className="text-center py-12 text-muted-foreground">
                    ...
                    <h5 className="font-medium">
                      {dictionary.testDetail.noAttachments}
                    </h5>
                    <p className="text-sm">
                      {dictionary.testDetail.attachmentsDesc}
                    </p>
                  </div>
                )}
              </ScrollArea>
