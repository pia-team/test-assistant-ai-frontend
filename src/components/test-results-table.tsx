...
const getStatusConfig = (dictionary: any) => ({
  completed: {
    ...
    bg: "bg-green-500/10",
    label: dictionary.jobDashboard?.successful || "Completed",
  },
  completed_with_failures: {
    icon: AlertCircle,
    ...
    bg: "bg-orange-500/10",
    label:
      dictionary.testRun?.completedWithFailures || "Completed with Failures",
  },
  running: {
    icon: Loader2,
    color: "text-blue-500",
    bg: "bg-blue-500/10",
    label: dictionary.testRun?.running || "Running",
    animate: true,
  },
  failed: {
    icon: XCircle,
    color: "text-red-500",
    bg: "bg-red-500/10",
    label: dictionary.testRun?.error || "Failed",
  },
  pending: {
    icon: Clock,
    color: "text-yellow-500",
    bg: "bg-yellow-500/10",
    label: dictionary.backgroundProcess?.pending || "Pending",
  },
  passed: {
    icon: CheckCircle2,
    color: "text-green-500",
    bg: "bg-green-500/10",
    label: dictionary.testDetail?.pass || "Passed",
  },
  skipped: {
    icon: Clock,
    color: "text-gray-500",
    bg: "bg-gray-500/10",
    label: dictionary.testList?.skipped || "Skipped",
  },
  stopped: {
    icon: XCircle,
    color: "text-orange-500",
    bg: "bg-orange-500/10",
    label: dictionary.testRun?.aborted || "Stopped",
  },
  unknown: {
    icon: Clock,
    color: "text-gray-400",
    bg: "bg-gray-400/10",
    label: dictionary.jobDashboard?.unknown || "Unknown",
  },
});

const LogViewer = React.memo(
  ({
    logs,
    status,
    compact = false,
        ...
        >
          <AnimatePresence mode="popLayout">
            {logs && logs.length > 0 ? (
              ...
              <div className="text-zinc-600 italic text-[11px] py-1">
                No logs available for this feature yet...
              </div>
            )}
          </AnimatePresence>
        </div>
...
export function TestResultsTable({
  ...
  const StatusBadge = ({
    ...
    const config =
      (getStatusConfig(dictionary) as Record<string, any>)[status] ||
      getStatusConfig(dictionary).unknown;
      ...
      <CardHeader>
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <CardDescription>
              {dictionary.testRun?.testResultsDesc ||
                    ...
                    <AnimatePresence>
                      {expandedRows.has(creation.id) && (
                        <TableRow key={`${creation.id}-detail`}>
                          <TableCell
                            colSpan={7}
                            className="p-0 bg-muted/10 border-y"
                          >
                            <motion.div
                                    ...
                                    <TabsContent value="logs">
                                      <div className="flex flex-col items-center justify-center py-12 text-slate-400 bg-zinc-50 dark:bg-zinc-900/50 rounded-xl border border-dashed border-zinc-200 dark:border-zinc-800">
                                        ...
                                        <p className="text-sm font-medium">
                                          Waiting for feature-specific logs...
                                        </p>
                                      </div>
                                    </TabsContent>
                                  )}
...
function TestResultDetailsModal({
  testId,
  creations,
  isOpen,
  onClose,
  ...
  const config =
    (getStatusConfig(dictionary) as Record<string, any>)[test.status] ||
    getStatusConfig(dictionary).unknown;
      ...
      <DialogContent className="w-[800px] sm:max-w-[800px] max-w-[calc(100vw-2rem)] max-h-[90vh] flex flex-col bg-white/95 dark:bg-zinc-950/95 backdrop-blur-xl border-zinc-200/50 dark:border-zinc-800/50 p-0 overflow-hidden shadow-2xl rounded-[24px]">
          ...
          <div className="space-y-4">
            ...
            <div className="aspect-video bg-black rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-2xl ring-1 ring-zinc-200/50 dark:ring-zinc-800/50">
              {(() => {
                ...
                const isRunning = ["running", "pending", "unknown"].includes(
                  test.status,
                );
                ...
                const showLoading =
                  isRunning || (expectsVideo && !test.videoUrl);
                ...
                if (showLoading) {
                    ...
                    <div className="flex flex-col items-center justify-center h-full gap-4 bg-gradient-to-br from-zinc-900 to-zinc-950">
                      ...
                      <div className="text-center space-y-2">
                        ...
                        <p className="text-xs text-zinc-500 font-mono">
                          {isRunning
                            ? "Video test tamamlandığında hazırlanacak"
                            : "Kayıt tamamlanmak üzere, lütfen bekleyin"}
                        </p>
                      </div>
                  ...
                  return (
                    <video
                      key={test.videoUrl}
                      src={
                        test.videoUrl.startsWith("http")
                          ? test.videoUrl
                          : `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080"}${test.videoUrl.startsWith("/") ? "" : "/"}${test.videoUrl}`
                      }
                      controls
                      autoPlay
                      playsInline
                  ...
                  return (
                    <div className="flex flex-col items-center justify-center h-full gap-4 text-zinc-500">
                      ...
                      <p className="text-sm font-mono opacity-50">
                        Video kaydı bu oturum için mevcut değil
                      </p>
                    </div>
                  );
                }
          ...
          {test.screenshotUrl && (
            <div className="space-y-4">
              ...
              <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-lg group cursor-zoom-in">
                <img
                  src={
                    test.screenshotUrl.startsWith("http")
                      ? test.screenshotUrl
                      : `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080"}${test.screenshotUrl}`
                  }
                  alt="Test Screenshot"
        ...
        <div className="p-5 border-t border-zinc-100 dark:border-zinc-900 bg-zinc-50/80 dark:bg-zinc-900/40 flex justify-between items-center px-8">
          ...
          <Button
            ...
            size="lg"
            onClick={onClose}
            className="px-12 font-bold transition-all hover:bg-zinc-200 dark:hover:bg-zinc-800"
          >
            Close
          </Button>
        </div>
...
function ModalStatusBadge({
  ...
  return (
    <Badge
      variant="outline"
      className={cn("gap-2 py-1 px-3 border-2", config.color, config.bg)}
    >
