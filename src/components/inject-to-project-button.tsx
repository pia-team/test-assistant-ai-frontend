...
export function InjectToProjectButton({
  ...
  const displayLabel =
    label || (useDefaultLabel ? dictionary.injection.addToProject : "");
  ...
  const {
    ...
    onSuccess: (result) => {
      ...
      toast.success(
        dictionary.injection.successMessage.replace(
          "{count}",
          String(result.injectedFiles.length),
        ),
  ...
  const handleModalOpenChange = (open: boolean) => {
    ...
    if (!open && hasCompleted && lastResultRef.current) {
      console.log(
        "[InjectToProjectButton] Modal closed after success, triggering parent onSuccess",
      );
