...
export function FileManagerPanel() {
  ...
  const getFileNameFromJob = (job: Job) => {
    try {
      // @ts-expect-error - request field might not be strictly typed in Job interface
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {(stats?.videoCount || 0) + (stats?.reportCount || 0)} toplam
              dosya
            </p>
          </CardContent>
        </Card>
                        ...
                        <PaginationContent>
                          ...
                          <PaginationItem>
                            <PaginationNext
                              onClick={() =>
                                setHistoryPage((p) =>
                                  Math.min(jobsData.totalPages - 1, p + 1),
                                )
                              }
                              className={
                                historyPage === jobsData.totalPages - 1
                                  ? "pointer-events-none opacity-50"
                                  : "cursor-pointer"
                              }
                            />
