...
export async function getTagsByProjectAction(project: string) {
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(
      `Failed to fetch tags for project ${project}: ${errorText}`,
    );
  }
...
export async function getFilesByGroupAction(groupName: string) {
  ...
  const response = await fetch(
    `${API_URL}/api/projects/${encodeURIComponent(groupName)}/features`,
    {
      method: "GET",
      headers: {
        ...(token && { Authorization: `Bearer ${token}` }),
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(
      `Failed to fetch files for group ${groupName}: ${errorText}`,
    );
  }
