...
export async function getPlaywrightConfig(
  ...
  const response = await fetch(
    `${API_URL}/api/playwright-config?environment=${environment}`,
    {
      method: "GET",
      headers: {
  ...
  if (!response.ok) {
    const error = await response
      .json()
      .catch(() => ({ message: "Failed to get config" }));
...
export async function updatePlaywrightConfig(
  config: PlaywrightConfigRequest,
): Promise<{ success: boolean; message: string; config?: PlaywrightConfig }> {
  const cookieStore = await cookies();
  const token = cookieStore.get("auth_token")?.value;
...
export async function getAvailableEnvironments(): Promise<string[]> {
  ...
  const response = await fetch(
    `${API_URL}/api/playwright-config/environments`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
