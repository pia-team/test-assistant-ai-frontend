...
export async function injectCode(
    ...
    try {
        ...
        if (!contentType || !contentType.includes("application/json")) {
            ...
            console.error(`[injectCode] Invalid response from backend (${response.status}): ${text.substring(0, 200)}...`);
            return {
                success: false,
                injectedFiles: [],
                conflicts: [],
        ...
        if (!response.ok) {
            ...
            return {
                ...
                message: result.message || `Injection failed with status ${response.status}`,
            };
        }
        ...
        return {
            ...
            message: error instanceof Error ? `System Error: ${error.message}` : "System Error",
        };
    }
}
...
export async function checkConflicts(
    ...
    try {
        ...
        if (!response.ok) {
            console.error(`[checkConflicts] Backend error (${response.status}):`, result);
            return {
                success: false,
                injectedFiles: [],
                conflicts: [],
...
export async function resolveConflict(
    ...
    try {
        ...
        if (!response.ok) {
            console.error(`[resolveConflict] Backend error (${response.status}):`, result);
            return {
                success: false,
                injectedFiles: [],
                conflicts: [],
