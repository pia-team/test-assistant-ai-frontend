...
const envSchema = z.object({
  ...
  baseLoginUrl: z
    .string()
    .min(1, "Giriş URL gereklidir")
    .refine(
      (val) => val.startsWith("http"),
      "Geçerli bir URL (http/https) giriniz",
    ),
...
export function EnvironmentsClient({ dictionary }: { dictionary: any }) {
  ...
  const fetchEnvironments = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080"}/api/environments`,
      );
      if (res.ok) {
  ...
  const onDelete = async (id: string, name: string) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080"}/api/environments/${id}`,
        {
          method: "DELETE",
        },
      );
        ...
        <CardHeader className="bg-slate-50/50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-800">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <CardDescription>
                Toplam {environments.length} profil bulundu
              </CardDescription>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-slate-500 max-w-xs mx-auto mb-6">
                {searchTerm
                  ...
                  : "Henüz herhangi bir ortam profili oluşturmadınız."}
              </p>
              {!searchTerm && (
                <Button
                  variant="outline"
                    ...
                    {filteredEnvironments.map((env) => (
                      ...
                      >
                        ...
                        <TableCell>
                          ...
                          >
                            {ENV_OPTIONS.find((o) => o.value === env.envKey)
                              ?.label || env.envKey}
                          </Badge>
                        </TableCell>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        placeholder="Örn: Müşteri Paneli Prod"
                        {...field}
                      />
                    </FormControl>
                    <FormMessage />
