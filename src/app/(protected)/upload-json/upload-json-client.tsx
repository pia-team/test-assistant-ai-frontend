...
export function UploadJsonClient({ dictionary }: UploadJsonClientProps) {
  ...
  const formSchema = z.object({
    file: z.instanceof(File, {
      message:
        dictionary.uploadJson.fileRequired || "Dosya yüklenmesi zorunludur",
    }),
    fileBaseName: z
      .string()
      ...
      .regex(
        /^[a-zA-Z0-9_-]+$/,
        dictionary.uploadJson.fileNameRegexError ||
          "Sadece harf, rakam, tire ve alt çizgi kullanılabilir",
      ),
    tags: z
      .array(z.string())
      .min(
        1,
        dictionary.uploadJson.mandatoryTagsError ||
    ...
    try {
      if (msg.trim().startsWith("{")) {
        ...
        return (
          parsed.message ||
          parsed.error ||
          dictionary.uploadJson.serverError ||
  ...
  const displayedJob =
    isViewingHistorical && selectedPreviousJobId !== "none"
  ...
  useEffect(() => {
    ...
    if (currentStatus === "FAILED" && prevStatusRef.current !== "FAILED") {
      const errorMsg =
        ...
        dictionary.uploadJson.uploadFailure ||
        "Upload failed";
  ...
  const result: UploadJsonResponse | null =
    isComplete && displayedJob?.result
  ...
  const processSelectedFile = useCallback(
    (file: File) => {
      ...
      toast.success(
        `${dictionary.uploadJson.selectedFile || "Dosya seçildi:"} ${file.name}`,
      );
    },
    [form, dictionary],
  );
  ...
  const handleFileChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (
        ...
        toast.error(
          dictionary.uploadJson.validJsonOnly ||
            "Lütfen geçerli bir JSON/HAR dosyası seçin",
        );
      }
    },
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    const effectiveProjectName =
      ...
      (selectedProjectFolder !== "none" ? selectedProjectFolder : "") ||
      "";
    ...
    startJobMutation.mutate(formData, {
      onError: (error) => {
        if (error.message.startsWith("JOB_ALREADY_RUNNING:")) {
          toast.warning(
            dictionary.uploadJson.jobAlreadyRunning ||
  ...
  const handleAddCustomTag = () => {
    ...
    if (tagToAdd.includes(" ")) {
      toast.error(
        dictionary.uploadJson.tagNoSpace || "Etiket adında boşluk bulunamaz",
      );
      return;
    }
                    ...
                    <div className="flex-1 space-y-1">
                      <h3 className="font-bold text-xl text-slate-800 dark:text-slate-100 flex items-center gap-2">
                        {displayedJob?.stepKey
                          ? (fullDict.progressSteps as any)?.uploadJson?.[
                              displayedJob.stepKey
                            ] || displayedJob.stepKey
                ...
                <CardContent className="p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <p className="font-semibold text-amber-800 dark:text-amber-200">
                      {dictionary.uploadJson.takingLonger ||
                        "İşlem Beklenenden Uzun Sürüyor"}
                    </p>
                    <p className="text-sm text-amber-700 dark:text-amber-300">
                      {dictionary.uploadJson.checkConnection ||
                ...
                <CardContent className="p-6 relative z-10 flex items-center gap-5">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-emerald-700 dark:text-emerald-300 font-medium">
                      {dictionary.uploadJson.filesCreatedSuccess ||
                        "Dosyalar başarıyla oluşturuldu."}
                    </p>
                  </div>
                  <Button
                    onClick={handleNewUpload}
                    className="bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg shadow-emerald-500/30 transition-transform hover:scale-105"
                  >
                    <Upload className="mr-2 h-4 w-4" />{" "}
                    {dictionary.uploadJson.uploadNewFile || "Yeni Dosya Yükle"}
                  </Button>
                </CardContent>
              </Card>
            </motion.div>
              ...
              <Card className="border-0 shadow-xl bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm overflow-hidden">
                <CardHeader className="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                  <CardTitle className="text-lg flex items-center gap-3">
                    ...
                    {dictionary.uploadJson.fileAndConfig ||
                      "Dosya ve Konfigürasyon"}
                  </CardTitle>
                  <CardDescription>
                    {dictionary.uploadJson.fileAndConfigDesc ||
                      "Yüklemek istediğiniz HAR veya JSON dosyasını seçin."}
                  </CardDescription>
                </CardHeader>
                <CardContent className="p-6 space-y-8">
                  ...
                  <div
                    ...
                    className={`group relative border-2 border-dashed rounded-2xl p-10 text-center cursor-pointer transition-all duration-300 hover:border-blue-500 hover:bg-blue-50/50 dark:hover:bg-blue-900/10 ${
                      isDragging
                  ...
                  >
                    ...
                    >
                      {form.watch("file") ? (
                        ...
                        >
                          ...
                          >
                            {dictionary.uploadJson.clickToChange ||
                        ...
                        <div className="space-y-4">
                          ...
                          <div>
                            <p className="font-medium text-lg text-slate-700 dark:text-slate-300">
                              {dictionary.uploadJson.dragFileHere ||
                                "Dosyayı buraya sürükleyin"}
                            </p>
                            <p className="text-sm text-slate-500">
                              {dictionary.uploadJson.clickToSelect ||
                                "veya seçmek için tıklayın (.har, .json)"}
                            </p>
                          </div>
                        </div>
                      )}
                    ...
                    {form.formState.errors.file && (
                      <div className="absolute bottom-2 left-0 w-full text-center">
                        <span className="text-red-500 text-sm font-medium bg-red-50 dark:bg-red-900/50 px-3 py-1 rounded-full">
                  ...
                  <div className="space-y-6">
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-base font-semibold text-slate-800 dark:text-slate-200">
                            {dictionary.uploadJson.fileNameLabel ||
                          ...
                          <FormControl>
                            <div className="relative">
                              <Input
                                ...
                                placeholder={
                                  dictionary.uploadJson.fileNamePlaceholder ||
                                  "Örn: login_flow"
                                }
                                disabled={isProcessing}
                              />
                          ...
                          <FormDescription className="text-xs">
                            {dictionary.uploadJson.fileNameDescription ||
                              "Bu isim, oluşturulacak feature ve page dosyalarının prefix'i olacaktır. Dosya seçince otomatik dolar."}
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-100 dark:border-slate-800">
                        ...
                        render={({ field }) => (
                          ...
                          >
                            <FormLabel className="flex items-center gap-2">
                              ...
                              {dictionary.uploadJson.addToExistingFolder ||
                                "Mevcut Klasöre Ekle"}
                            </FormLabel>
                            <Select
                            ...
                            >
                              ...
                              <SelectContent>
                                <SelectItem value="none">
                                  {dictionary.uploadJson.noSelection ||
                                    "Seçim Yok"}
                                </SelectItem>
                      ...
                      <div className="flex items-center justify-center text-xs font-bold text-slate-400 md:hidden">
                        {dictionary.uploadJson.or || "- VEYA -"}
                      </div>
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem
                            className={
                              form.watch("selectedProjectFolder") !== "none"
                                ? "opacity-50 pointer-events-none transition-opacity"
                                : "transition-opacity"
                            }
                          >
                            <FormLabel className="flex items-center gap-2">
                              ...
                              {dictionary.uploadJson.createNewFolder ||
                                "Yeni Klasör Oluştur"}
                            </FormLabel>
                            <FormControl>
                              <Input
                                ...
                                placeholder={
                                  dictionary.uploadJson.folderNamePlaceholder ||
                                  "Klasör adı giriniz..."
                                }
                                disabled={isProcessing}
                              />
                ...
                <CardHeader className="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-lg flex items-center gap-3">
                      ...
                      {dictionary.uploadJson.tagManagement ||
                        "Etiket (Tag) Yönetimi"}{" "}
                      <span className="text-red-500">*</span>
                    </CardTitle>
                    <Badge variant="outline" className="font-mono text-xs">
                      {form.watch("tags").length}{" "}
                      {dictionary.uploadJson.selected || "Seçildi"}
                    </Badge>
                  </div>
                </CardHeader>
                      ...
                      <FormItem className="flex-1 flex-col">
                        <div className="relative mb-4">
                          <Input
                            ...
                            placeholder={
                              dictionary.uploadJson.searchOrAddTagPlaceholder ||
                              "Etiket arayın veya ekleyin (@smoke, @reg)..."
                            }
                        ...
                        <div className="min-h-[100px] p-4 bg-slate-50 dark:bg-slate-950/50 rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-800 mb-6">
                          {form.watch("tags").length === 0 ? (
                            <div className="h-full flex flex-col items-center justify-center text-slate-400 text-sm italic">
                              ...
                              {dictionary.uploadJson.noTagsSelected ||
                                "Henüz etiket seçilmedi. En az bir tane zorunlu."}
                            </div>
                          ) : (
                          ...
                          <div className="flex flex-wrap gap-2 max-h-[200px] overflow-y-auto custom-scrollbar pr-2">
                            ...
                            {existingTags.length === 0 && (
                              <span className="text-sm text-slate-400">
                                {dictionary.uploadJson.noSavedTags ||
                                  "Kayıtlı etiket bulunamadı."}
                              </span>
                            )}
                          </div>
                        </div>
        ...
        >
          <div className="flex items-center gap-4 mb-6">
            ...
            <h2 className="text-2xl font-bold text-slate-700 dark:text-slate-300">
              {dictionary.uploadJson.generatedFiles || "Oluşturulan Dosyalar"}
            </h2>
            <div className="h-px bg-slate-200 dark:bg-slate-800 flex-1" />
          </div>
