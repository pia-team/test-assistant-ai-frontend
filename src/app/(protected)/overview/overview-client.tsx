...
export function OverviewClient({ dictionary }: OverviewClientProps) {
    ...
    const outputTypes = [
        { name: out.featureFiles, description: out.featureFilesDesc, icon: FileCode },
        { name: out.stepDefinitions, description: out.stepDefinitionsDesc, icon: Code2 },
        { name: out.apiTestData, description: out.apiTestDataDesc, icon: FileJson },
        { name: out.allureReports, description: out.allureReportsDesc, icon: BarChart3 }
    ];
        ...
        <div className="space-y-24 pb-16">
            ...
            <section className="relative overflow-hidden">
                ...
                >
                    ...
                    <p className="text-xl text-muted-foreground max-w-3xl mx-auto leading-relaxed">
                        {hero.description}
                        <strong className="text-foreground"> {hero.descriptionStrong}</strong>
                    </p>
            ...
            <section className="space-y-12">
                <motion.div 
                    ...
                    {...fadeInUp}
                >
                    <Badge variant="outline" className="px-4 py-1">{f.badge}</Badge>
                    <h2 className="text-3xl sm:text-4xl font-bold">
                        {f.title?.split(" ").slice(0, -1).join(" ")} <span className="text-primary">{f.title?.split(" ").slice(-1)}</span>
                    </h2>
            ...
            <section id="how-it-works" className="space-y-12 scroll-mt-20">
                <motion.div 
                    ...
                    {...fadeInUp}
                >
                    <Badge variant="outline" className="px-4 py-1">{h.badge}</Badge>
                    <h2 className="text-3xl sm:text-4xl font-bold">
                        <span className="text-primary">{h.title?.split(" ").slice(0, 2).join(" ")}</span> {h.title?.split(" ").slice(2).join(" ")}
                    </h2>
                    <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
                        {h.subtitle}
                    </p>
                    ...
                    <div className="relative">
                        ...
                        >
                            {steps.map((step, index) => (
                                ...
                                >
                                    {/* Step Number */}
                                    <div className={`flex-shrink-0 w-16 h-16 rounded-2xl ${step.color} flex items-center justify-center shadow-lg z-10`}>
            ...
            <section className="space-y-12">
                <motion.div 
                    ...
                    {...fadeInUp}
                >
                    <Badge variant="outline" className="px-4 py-1">{out.badge}</Badge>
                    <h2 className="text-3xl sm:text-4xl font-bold">
                        <span className="text-primary">{out.title?.split(" ").slice(0, 1).join(" ")}</span> {out.title?.split(" ").slice(1).join(" ")}
                    </h2>
                    <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
                        {out.subtitle}
                    </p>
    ...
    And the user should be created successfully`}</code>
            ...
            <section className="space-y-12">
                <motion.div 
                    ...
                    {...fadeInUp}
                >
                    <Badge variant="outline" className="px-4 py-1">{tech.badge}</Badge>
                    <h2 className="text-3xl sm:text-4xl font-bold">
                        {tech.title?.split(" ").slice(0, 1).join(" ")} <span className="text-primary">{tech.title?.split(" ").slice(1).join(" ")}</span>
                    </h2>
                </motion.div>
                ...
                >
                    {[
                        "Playwright", "Cucumber", "TypeScript", "Gherkin", 
                        "Allure Reports", "Google AI", "Next.js", "Spring Boot"
                    ].map((techItem, index) => (
                        <Badge 
                            key={index} 
                            variant="secondary" 
            ...
            <section className="relative">
                ...
                >
                    ...
                    <h2 className="text-3xl sm:text-4xl font-bold">
                        {cta.title?.split(" ").slice(0, -1).join(" ")} <span className="text-primary">{cta.title?.split(" ").slice(-1)}</span>
                    </h2>
