...
const isValidSwaggerUrl = (url: string): boolean => {
  ...
  try {
    ...
    return swaggerPatterns.some(
      (pattern) => pattern.test(pathname) || pattern.test(href),
    );
  } catch {
    return false;
  }
...
export function GenerateTestsClient({ dictionary }: GenerateTestsClientProps) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      toast.success(
        dictionary.generateTests.testsGeneratedSuccess ||
    ...
    if (currentStatus === "FAILED" && prevStatusRef.current !== "FAILED") {
      const errMsg =
        ...
        dictionary.generateTests.errorGeneratingTests ||
  ...
  const result: GeneratedResult | null =
  ...
  const onSubmit = (data: GenerateTestsFormValues) => {
    if (!isValidSwaggerUrl(data.url)) {
      form.setError("url", {
        message: "Lütfen geçerli bir Swagger/OpenAPI URL'si girin",
      });
      return;
    }
      ...
      },
      {
        onError: (error) => {
          if (error.message.startsWith("JOB_ALREADY_RUNNING:")) {
            toast.warning(
              dictionary.generateTests.jobAlreadyRunning ||
  ...
  const canGenerate =
    url.trim() && isValidSwaggerUrl(url) && !isProcessing && !errors.url;
                      ...
                      <h3 className="font-bold text-xl text-slate-800 dark:text-slate-100 flex items-center gap-2">
                        {currentJob?.stepKey
                          ? (
                              ...
                              >
                            )?.generateTests?.[currentJob.stepKey] ||
                            currentJob.stepKey
                          : dictionary.generateTests.processingInBackground ||
          ...
          <CardContent className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Örnek: https://api.example.com/swagger.json, /v2/api-docs,
                /openapi.yaml
              </p>
            </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <Label className="font-normal">
                    {dictionary.generateTests.featureFile}
                  </Label>
                </div>
                <div className="flex items-center gap-3">
                  <Switch
                  ...
                  />
                  <Label className="font-normal">
                    {dictionary.generateTests.apiTests}
                  </Label>
                </div>
                <div className="flex items-center gap-3">
                  <Switch
                  ...
                  />
                  <Label className="font-normal">
                    {dictionary.generateTests.testPayload}
                  </Label>
                </div>
                <div className="flex items-center gap-3">
                  <Switch
                  ...
                  />
                  <Label className="font-normal">
                    {dictionary.generateTests.swaggerTests}
                  </Label>
                </div>
              </div>
            </div>
        ...
        <Card className="lg:col-span-3">
          <CardHeader>
            <CardTitle className="text-lg">
              {dictionary.generateTests.generatedResults}
            </CardTitle>
          </CardHeader>
          <CardContent>
            {!result ? (
