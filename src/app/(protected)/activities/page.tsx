...
export default async function HomePage() {
    ...
    const activity = (dict as Record<string, Record<string, string>>).activity || {};
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div className="space-y-1">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-3xl font-bold">{activity.title || "Test Activities"}</h1>
                    </div>
                    <p className="text-muted-foreground">
                        {activity.subtitle || "Real-time status and history of all test operations"}
                    </p>
                </div>
            </div>
                ...
                <Link href="/generate-tests">
                    <Card className="group hover:shadow-md hover:border-primary/30 transition-all cursor-pointer">
                        <CardContent className="p-4 flex items-center gap-4">
                            ...
                            <div>
                                <h3 className="font-semibold">{dict.nav?.generateTests || "Generate Tests"}</h3>
                ...
                <Link href="/upload-json">
                    <Card className="group hover:shadow-md hover:border-primary/30 transition-all cursor-pointer">
                        <CardContent className="p-4 flex items-center gap-4">
                            ...
                            <div>
                                <h3 className="font-semibold">{dict.nav?.uploadJson || "Upload JSON"}</h3>
                                <p className="text-sm text-muted-foreground">JSON/HAR file analysis</p>
                            </div>
                        </CardContent>
                    </Card>
                </Link>
                <Link href="/test-run">
                    <Card className="group hover:shadow-md hover:border-primary/30 transition-all cursor-pointer">
                        <CardContent className="p-4 flex items-center gap-4">
                            ...
                            <div>
                                <h3 className="font-semibold">{dict.nav?.testRun || "Run Tests"}</h3>
                                <p className="text-sm text-muted-foreground">Execute Playwright tests</p>
                            </div>
                        </CardContent>
                    </Card>
                </Link>
